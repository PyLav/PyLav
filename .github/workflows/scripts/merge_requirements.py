...
for name in names:
    ...
    for req_name, platforms in input_data.items():
        ...
        base_req = next((base_req for base_req in base_requirements if base_req.name == req.name), None)
        ...
        if len(platforms) == len(all_platforms):
            ...
            platform_marker = " or ".join(f"sys_platform == '{platform}'" for platform in platforms)
        else:
            platform_marker = " and ".join(
                f"sys_platform != '{platform}'" for platform in all_platforms - platforms.keys()
            )

        new_marker = f"({req.marker}) and ({platform_marker})" if req.marker is not None else platform_marker
