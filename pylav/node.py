...
from pylav.exceptions import Unauthorized, UnsupportedNodeAPI
...
class Node:
    ...
    async def decode_track(self, track: str, timeout: ClientTimeout | object = sentinel) -> TrackObject:
        async with self.session.get(
            self.get_endpoint_decodetrack(), headers={"Authorization": self.password}, params={"track": track}, timeout=timeout
        ) as res:
            if res.status == 200:
