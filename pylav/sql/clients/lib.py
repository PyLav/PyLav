...
class LibConfigManager:
    ...
    async def create_tables() -> None:
        ...
        await pylav.sql.tables.lib_config.LibConfigRow.raw(
            ...
            f"ON {pylav.sql.tables.lib_config.LibConfigRow._meta.tablename} (bot, id)"
        )
        ...
        await pylav.sql.tables.player_states.PlayerStateRow.raw(
            ...
            f"ON {pylav.sql.tables.player_states.PlayerStateRow._meta.tablename} (bot, id)"
        )
        await pylav.sql.tables.players.PlayerRow.create_table(if_not_exists=True)
        await pylav.sql.tables.players.PlayerRow.raw(
            f"CREATE UNIQUE INDEX IF NOT EXISTS unique_player_bot_id "
            f"ON {pylav.sql.tables.players.PlayerRow._meta.tablename} (bot, id)"
        )
