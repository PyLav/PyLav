...
from pylav.vendored.aiocache import serializers
...
class API:
    ...
    def plugins(cls, func):
        ...
        async def _plugins(self, *args, **kwargs):
            ...
            for plugin in self.plugins:
                await getattr(plugin, f"post_{func.__name__}")(self, *args, took=end - start, ret=ret, **kwargs)
            return ret
...
class BaseCache:
    ...
    def __init__(self, serializer=None, plugins=None, namespace=None, key_builder=None, timeout=5, ttl=None):
    ...
    async def multi_get(self, keys, loads_fn=None, namespace=None, _conn=None):
        ...
        values = [
            loads(value) for value in await self._multi_get(ns_keys, encoding=self.serializer.encoding, _conn=_conn)
        ]
    ...
    async def set(self, key, value, ttl=SENTINEL, dumps_fn=None, namespace=None, _cas_token=None, _conn=None):
        ...
        res = await self._set(ns_key, dumps(value), ttl=self._get_ttl(ttl), _cas_token=_cas_token, _conn=_conn)
