...
from pylav.vendored.aiocache import AIOCACHE_CACHES
from pylav.vendored.aiocache.base import BaseCache
from pylav.vendored.aiocache.exceptions import InvalidCacheType
...
class Cache:
    ...
    def __new__(cls, cache_class=MEMORY, **kwargs):
        if not issubclass(cache_class, BaseCache):
            raise InvalidCacheType(f"Invalid cache type, you can only use {list(AIOCACHE_CACHES.keys())}")
    ...
    def get_scheme_class(cls, scheme):
        try:
            ...
            raise InvalidCacheType(f"Invalid cache type, you can only use {list(AIOCACHE_CACHES.keys())}") from e
...
class CacheHandler:
    ...
    _config: dict[str, dict[str, object]] = {
        "default": {
            ...
            "serializer": {"class": "pylav.vendored.aiocache.serializers.StringSerializer"},
        }
    }
