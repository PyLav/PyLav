...
if TYPE_CHECKING:  # keep mypy quiet
    from pylav.vendored.aiopath.aiopath_310.path import AsyncPath
...
class IterableAIOFile(FileLike, AIOFile):
    def __init__(self, *args, errors: str | None = ERRORS, newline: str | None = SEP, **kwargs):
    ...
    def _get_options(self, encoding: str | None = None, errors: str | None = None) -> tuple[str, str, str]:
    ...
    async def read_text(self, encoding: str | None = None, errors: str | None = None) -> str:
        ...
        return await read_full_file(self.name, line_sep, encoding=encoding, errors=errors)

    async def read(self, size: int = -1, offset: int | None = None) -> FileData:
        if offset is None:
            offset = self._offset
...
async def read_lines(
    ...
    **kwargs,
    ...
    async with AIOFile(path, "rb") as handle:
        reader = LineReader(handle, line_sep=line_sep, chunk_size=chunk_size, offset=offset)
...
async def read_full_file(
    ...
    **kwargs,
    ...
    lines_gen = read_lines(
        path, line_sep=line_sep, chunk_size=chunk_size, offset=offset, encoding=encoding, errors=errors
    )
